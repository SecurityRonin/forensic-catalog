...
def parse_sans_blog_html(html_text: str) -> list[tuple[str, str, str]]:
    """Extract blog posts from a SANS blog page.

    SANS uses Remix SSR + ContentStack CMS. Post data is embedded in
    window.__remixContext as JSON — no browser required.

    Returns list of (title, url, date) where:
      url  = absolute https://www.sans.org/blog/... URL
      date = YYYY-MM-DD
    """
    if not html_text:
        return []
    # Find the hits array in the embedded JSON blob (Remix SSR / ContentStack)
    start_marker = '"hits":['
    idx = html_text.find(start_marker)
    if idx == -1:
        return []
    # Walk forward to find the balanced closing ] of the hits array
    array_start = idx + len(start_marker) - 1  # points to the opening [
    depth = 0
    array_end = -1
    ...
    for h in hits:
        ...
        date = created[:10]  # YYYY-MM-DD from ISO 8601
        entries.append((title, url_path, date))
    return entries
