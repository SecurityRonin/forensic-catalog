...
pub static BEURK: MalwareProfile = MalwareProfile {
    ...
    signals: &[
        ...
        ProfileSignal { id: ELF_HOOKS_FILE_HIDING,           weight: 25, required: true  },
        ProfileSignal { id: ELF_HOOKS_NETWORK_HIDING,        weight: 25, required: true  },
        ProfileSignal { id: ELF_HOOKS_PAM_CREDENTIAL,        weight: 20, required: false },
        ...
        ProfileSignal { id: ELF_LIBC_SHADOW_EXPORTS,         weight: 10, required: false },
        ProfileSignal { id: TEMPORAL_LDPRELOAD_SSHD_RESTART, weight: 10, required: false },
    ],
    exclusions: &[
        WeightedExclusion { id: ARTIFACT_PAM_STAGING_STRUCTURAL, penalty: 35 },
        WeightedExclusion { id: ARTIFACT_PAM_STAGING_FATHER,     penalty: 40 },
...
mod tests {
    ...
    fn beurk_required_signals_reach_class_threshold() {
        let s = sigs(&[ELF_HOOKS_PROCESS_HIDING, ELF_HOOKS_FILE_HIDING, ELF_HOOKS_NETWORK_HIDING]);
        let m = score_against_profile(&s, &BEURK);
        assert!(m.score >= BEURK.class_threshold,
            "score {} < class_threshold {}", m.score, BEURK.class_threshold);
    ...
    fn beurk_with_magic_packet_reaches_probable() {
        let s = sigs(&[ELF_HOOKS_PROCESS_HIDING, ELF_HOOKS_FILE_HIDING, ELF_HOOKS_NETWORK_HIDING,
                       NETWORK_MAGIC_PACKET_KNOCK, ELF_GLOBALLY_LOADED]);
        let m = score_against_profile(&s, &BEURK);
        assert!(m.score >= BEURK.probable_threshold,
            "score {} < probable_threshold {}", m.score, BEURK.probable_threshold);
    ...
    fn beurk_full_signals_reach_confirmed() {
        let s = sigs(&[ELF_HOOKS_PROCESS_HIDING, ELF_HOOKS_FILE_HIDING, ELF_HOOKS_NETWORK_HIDING,
                       NETWORK_MAGIC_PACKET_KNOCK, ELF_GLOBALLY_LOADED, ELF_NOT_IN_PKG_DB,
                       ARTIFACT_LD_PRELOAD_FOREIGN, ELF_LIBC_SHADOW_EXPORTS,
    ...
    fn beurk_father_pam_staging_reduces_score() {
        let base = sigs(&[ELF_HOOKS_PROCESS_HIDING, ELF_HOOKS_FILE_HIDING, ELF_HOOKS_NETWORK_HIDING,
                          NETWORK_MAGIC_PACKET_KNOCK]);
        let with_father = sigs(&[ELF_HOOKS_PROCESS_HIDING, ELF_HOOKS_FILE_HIDING, ELF_HOOKS_NETWORK_HIDING,
                                  NETWORK_MAGIC_PACKET_KNOCK, ARTIFACT_PAM_STAGING_FATHER]);
        let m_base   = score_against_profile(&base, &BEURK);
        let m_father = score_against_profile(&with_father, &BEURK);
        assert!(m_father.score < m_base.score, "Father PAM staging should penalise Beurk score");
    }

    #[test]
    fn beurk_kernel_taint_reduces_score() {
        let base = sigs(&[ELF_HOOKS_PROCESS_HIDING, ELF_HOOKS_FILE_HIDING, ELF_HOOKS_NETWORK_HIDING]);
        let with_lkm = sigs(&[ELF_HOOKS_PROCESS_HIDING, ELF_HOOKS_FILE_HIDING, ELF_HOOKS_NETWORK_HIDING,
                               SYSTEM_KERNEL_TAINT_OOT]);
        let m_base = score_against_profile(&base, &BEURK);
        let m_lkm  = score_against_profile(&with_lkm, &BEURK);
        assert!(m_lkm.score < m_base.score, "kernel taint should penalise Beurk (pure LD_PRELOAD)");
    }
}
