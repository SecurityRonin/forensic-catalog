...
pub static NECRO: MalwareProfile = MalwareProfile {
    ...
    signals: &[
        ProfileSignal { id: PROCESS_THREAD_MINER_XMRIG,   weight: 40, required: true  },
        ProfileSignal { id: ELF_STRING_STAGING_PATH,       weight: 15, required: true  },
        ProfileSignal { id: SYSTEM_CPU_ANOMALY_HIGH,       weight: 25, required: false },
        ...
        ProfileSignal { id: NETWORK_STRATUM_CONNECTION,    weight: 20, required: false },
    ...
    exclusions: &[
        ...
        WeightedExclusion { id: ARTIFACT_PAM_STAGING_STRUCTURAL, penalty: 20 },
    ],
    class_threshold:     55,
...
mod tests {
    ...
    fn necro_required_signals_reach_class_threshold() {
        ...
        assert!(m.score >= NECRO.class_threshold,
            "score {} < class_threshold {}", m.score, NECRO.class_threshold);
        assert!(m.classification >= Classification::ClassMatch);
    }

    #[test]
    fn necro_with_high_cpu_reaches_probable() {
        let s = sigs(&[PROCESS_THREAD_MINER_XMRIG, ELF_STRING_STAGING_PATH, SYSTEM_CPU_ANOMALY_HIGH]);
        let m = score_against_profile(&s, &NECRO);
        assert!(m.score >= NECRO.probable_threshold,
            "score {} < probable_threshold {}", m.score, NECRO.probable_threshold);
    ...
    fn necro_full_signals_reach_confirmed() {
        let s = sigs(&[PROCESS_THREAD_MINER_XMRIG, ELF_STRING_STAGING_PATH,
                       SYSTEM_CPU_ANOMALY_HIGH, NETWORK_STRATUM_CONNECTION,
                       PROCESS_HIDDEN_FROM_PS, PROCESS_MASQUERADE]);
        let m = score_against_profile(&s, &NECRO);
    ...
    fn necro_kernel_taint_reduces_score() {
        let base = sigs(&[PROCESS_THREAD_MINER_XMRIG, ELF_STRING_STAGING_PATH, SYSTEM_CPU_ANOMALY_HIGH]);
        let with_lkm = sigs(&[PROCESS_THREAD_MINER_XMRIG, ELF_STRING_STAGING_PATH,
        ...
        assert!(m_lkm.score < m_base.score, "kernel taint should penalise Necro (no LKM)");
    }
