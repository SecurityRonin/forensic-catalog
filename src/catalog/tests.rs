...
mod decode_tests {
    ...
    fn catalog_has_entries() {
        ...
        assert_eq!(CATALOG.list().len(), 6610);
    }
...
mod tests_batch_i_presence {
    ...
    fn catalog_count_includes_batch_i() {
        assert_eq!(
            CATALOG.list().len(),
            6610,
            "catalog must have 6610 entries after batch I + quicklook_thumbnails + windows_install_date + winscp_ini + macos_wifi_intelligence + windows_clipboard_history"
        );
    }
}
...
mod tests_quicklook_install_date {
    ...
    fn catalog_count_includes_quicklook_and_install_date() {
        assert_eq!(
            CATALOG.list().len(),
            6610,
            "catalog must have 6610 entries after adding quicklook_thumbnails + windows_install_date + winscp_ini + macos_wifi_intelligence + windows_clipboard_history"
        );
    }
}
...
mod tests_winscp_ini {
    ...
    fn catalog_count_includes_winscp_ini() {
        assert_eq!(
            CATALOG.list().len(),
            6610,
            "catalog must have 6610 entries after adding winscp_ini + macos_wifi_intelligence + windows_clipboard_history"
        );
    }
}
