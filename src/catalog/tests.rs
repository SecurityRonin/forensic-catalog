...
mod decode_tests {
    ...
    fn catalog_has_entries() {
        ...
        assert_eq!(CATALOG.list().len(), 6661);
    }
...
mod tests_batch_i_presence {
    ...
    /// Per Carvey's "Events Ripper Updates" (windowsir.blogspot.com, 2023-08-02),
    /// Event ID 5152 in Security.evtx records WFP-blocked packets and is a
    /// pivot for detecting EDR-silencing / firewall-block tampering and
    /// inbound recon attempts. Carvey added a `filter.pl` plugin specifically
    /// for this in his Events Ripper toolset.
    #[test]
    fn evtx_security_mentions_event_5152() {
        let d = CATALOG
            .by_id("evtx_security")
            .expect("evtx_security missing");
        assert!(
            d.meaning.contains("5152"),
            "evtx_security meaning should mention Event ID 5152 (WFP packet block)"
        );
    }

    /// Per Carvey's "Events Ripper Updates" (windowsir.blogspot.com, 2023-08-02),
    /// Service Control Manager Event ID 7031 (service crash) in System.evtx
    /// is an analyst pivot point: services that crash unexpectedly often
    /// correlate with attempts to tamper with EDR / AV agents or with
    /// failed persistence-via-service installs. Carvey added an `scm.pl`
    /// check for 7031.
    #[test]
    fn evtx_system_mentions_event_7031() {
        let d = CATALOG.by_id("evtx_system").expect("evtx_system missing");
        assert!(
            d.meaning.contains("7031"),
            "evtx_system meaning should mention Event ID 7031 (SCM service crash)"
        );
    }

    /// Per Carvey's "Devices" (windowsir.blogspot.com, 2026-02-02), the
    /// Microsoft-Windows-DriverFrameworks-UserMode/Operational channel — once
    /// a primary USB-connect/disconnect data source — is disabled by default
    /// on modern Windows and "doesn't provide much depth" even when enabled.
    /// The meaning text must call this out so analysts know to enable it
    /// (wevtutil sl ... /e:true) BEFORE they need it, and to fall back to
    /// USBSTOR / EMDMgmt / setupapi.dev.log / Microsoft-Windows-Partition
    /// Operational + MsiInstaller pivots when the channel is empty.
    #[test]
    fn evtx_driver_frameworks_meaning_mentions_disabled_by_default() {
        let d = CATALOG
            .by_id("evtx_driver_frameworks")
            .expect("evtx_driver_frameworks missing");
        let meaning_lower = d.meaning.to_ascii_lowercase();
        assert!(
            meaning_lower.contains("disabled by default"),
            "evtx_driver_frameworks meaning should warn that the channel is disabled by default"
        );
        assert!(
            meaning_lower.contains("wevtutil"),
            "evtx_driver_frameworks meaning should mention wevtutil to enable"
        );
        assert!(
            d.sources
                .iter()
                .any(|s| s.contains("windowsir.blogspot.com/2026/02/devices.html")),
            "evtx_driver_frameworks sources should cite Carvey 2026 Devices post"
        );
    }

    /// Per Carvey's "Events Ripper Updates" (windowsir.blogspot.com, 2023-08-02),
    /// the Microsoft-Windows-Windows Defender/Operational channel records
    /// Event IDs 2050/2051 (sample uploaded / cannot upload) plus the
    /// canonical 1116/1117 (malware detected/blocked) and 5007 (config
    /// changed). The `defender.pl` Events Ripper plugin pivots on 2050
    /// because it fingerprints exactly which file path + hash Defender
    /// shipped to the cloud — a high-value DFIR pivot when the original
    /// dropped binary has been deleted.
    #[test]
    fn catalog_has_evtx_defender_operational() {
        let d = CATALOG
            .by_id("evtx_defender_operational")
            .expect("evtx_defender_operational missing from catalog");
        assert!(
            d.meaning.contains("2050") && d.meaning.contains("2051"),
            "evtx_defender_operational meaning should mention 2050/2051 (Defender sample upload)"
        );
        assert!(
            d.meaning.contains("1116") && d.meaning.contains("1117"),
            "evtx_defender_operational meaning should mention 1116/1117 (malware detected/blocked)"
        );
    }
...
// ── AWS CloudTrail IAM Events (HECF blogs #810-#812) ────────────────────────

#[cfg(test)]
mod tests_aws_cloudtrail_iam_events {
    use super::*;
    ...
    #[test]
    fn meaning_mentions_iam_latency() {
        let d = CATALOG.by_id("aws_cloudtrail_iam_events").unwrap();
        assert!(
            d.meaning.contains("IAM") && d.meaning.contains("us-east-1"),
            "meaning should reference IAM events logged in us-east-1"
        );
    }

    #[test]
    fn meaning_mentions_event_names() {
        let d = CATALOG.by_id("aws_cloudtrail_iam_events").unwrap();
        assert!(
            d.meaning.contains("CreateUser"),
            "meaning should mention CreateUser eventName"
        );
        assert!(
            d.meaning.contains("AddUserToGroup"),
            "meaning should mention AddUserToGroup eventName"
        );
        assert!(
            d.meaning.contains("RemoveUserFromGroup"),
            "meaning should mention RemoveUserFromGroup eventName"
        );
    }
    ...
    fn has_mitre_techniques() {
        let d = CATALOG.by_id("aws_cloudtrail_iam_events").unwrap();
        assert!(
            d.mitre_techniques.contains(&"T1136.003"),
            "should map to Create Account: Cloud Account"
        );
        assert!(
            d.mitre_techniques.contains(&"T1098.001"),
            "should map to Account Manipulation: Additional Cloud Credentials"
        );
    }

    #[test]
    fn triage_priority_is_critical() {
        let d = CATALOG.by_id("aws_cloudtrail_iam_events").unwrap();
        assert_eq!(d.triage_priority, TriagePriority::Critical);
    }
    ...
    fn has_hecf_sources() {
        let d = CATALOG.by_id("aws_cloudtrail_iam_events").unwrap();
        assert!(
            d.sources.iter().any(|s| s.contains("hecfblog.com")),
            "sources must include HECF blog posts"
        );
        assert!(
            d.sources.iter().any(|s| s.contains("docs.aws.amazon.com")),
            "sources must include AWS documentation"
        );
    }

    #[test]
    fn related_artifacts_include_aws_credentials() {
        let d = CATALOG.by_id("aws_cloudtrail_iam_events").unwrap();
        assert!(
...
mod tests_macos_btm_background_tasks {
    ...
    fn has_fields_for_type_and_disposition() {
        ...
        let field_names: Vec<&str> = d.fields.iter().map(|f| f.name).collect();
        assert!(
            field_names.contains(&"item_type"),
            "must have item_type field"
        );
        assert!(
...
mod tests_enable_periodic_backup {
    ...
    fn enable_periodic_backup_related_to_regback_hive() {
        let d = CATALOG.by_id("enable_periodic_backup").unwrap();
        assert!(
            d.related_artifacts.iter().any(|r| r.contains("regback")),
...
// ── Windows Clipboard Data Files (Carvey "What's on your clipboard?" 2026-01) ──
//
// On-disk persisted clipboard history under %LOCALAPPDATA%\Microsoft\Windows\Clipboard\.
// This is the file-side counterpart to windows_clipboard_history (registry settings)
// and the cbdhsvc service process memory, all targeted by ClipboardHistoryThief.
// Pinned items create a subfolder structure with metadata.json containing copy
// timestamps; the payload files themselves are encrypted at rest.

#[cfg(test)]
mod tests_windows_clipboard_data_files {
    use super::*;

    #[test]
    fn windows_clipboard_data_files_exists() {
        assert!(
            CATALOG.by_id("windows_clipboard_data_files").is_some(),
            "catalog must contain 'windows_clipboard_data_files'"
        );
    }

    #[test]
    fn windows_clipboard_data_files_is_file() {
    ...
    #[test]
    fn windows_clipboard_data_files_is_win10_plus() {
        let d = CATALOG.by_id("windows_clipboard_data_files").unwrap();
        assert_eq!(
            d.os_scope,
            OsScope::Win10Plus,
            "Clipboard history is Win10 1809+"
        );
    }

    #[test]
    fn windows_clipboard_data_files_path_contains_clipboard_folder() {
        let d = CATALOG.by_id("windows_clipboard_data_files").unwrap();
        let path = d.file_path.unwrap_or("");
        assert!(
            path.contains(r"Microsoft\Windows\Clipboard"),
            "file_path must point at %LOCALAPPDATA%\\Microsoft\\Windows\\Clipboard, got {path}"
        );
    }

    #[test]
    fn windows_clipboard_data_files_user_scope() {
        let d = CATALOG.by_id("windows_clipboard_data_files").unwrap();
        assert_eq!(
            d.scope,
            DataScope::User,
            "clipboard data files live in per-user LOCALAPPDATA"
        );
    }

    #[test]
    fn windows_clipboard_data_files_mitre_t1115() {
        let d = CATALOG.by_id("windows_clipboard_data_files").unwrap();
        assert!(
            d.mitre_techniques.contains(&"T1115"),
            "windows_clipboard_data_files must map to T1115 (Clipboard Data)"
        );
    }

    #[test]
    fn windows_clipboard_data_files_has_metadata_field() {
        let d = CATALOG.by_id("windows_clipboard_data_files").unwrap();
        assert!(
            d.fields.iter().any(|f| f.name == "metadata_json"),
            "must enumerate the metadata.json field for pinned items"
        );
    }

    #[test]
    fn windows_clipboard_data_files_has_encrypted_payload_field() {
        let d = CATALOG.by_id("windows_clipboard_data_files").unwrap();
        assert!(
            d.fields.iter().any(|f| f.name == "encrypted_payload"),
            "must enumerate the encrypted payload field"
        );
    }

    #[test]
    fn windows_clipboard_data_files_related_includes_registry_settings() {
        let d = CATALOG.by_id("windows_clipboard_data_files").unwrap();
        assert!(
            d.related_artifacts.contains(&"windows_clipboard_history"),
            "must relate to windows_clipboard_history (registry toggle controls existence of this folder)"
        );
    }

    #[test]
    fn windows_clipboard_data_files_related_includes_timeline() {
        let d = CATALOG.by_id("windows_clipboard_data_files").unwrap();
        assert!(
            d.related_artifacts.contains(&"windows_timeline"),
            "must relate to windows_timeline (ActivitiesCache.db CopyPaste records)"
        );
    }

    #[test]
    fn windows_clipboard_data_files_cites_carvey_2026() {
        let d = CATALOG.by_id("windows_clipboard_data_files").unwrap();
        assert!(
            d.sources
                .iter()
                .any(|s| s.contains("windowsir.blogspot.com/2026/01/whats-on-your-clipboard.html")),
            "must cite Carvey 2026-01 'What's on your clipboard?' as source"
        );
    }

    #[test]
    fn windows_clipboard_data_files_cites_thinkdfir_clippy() {
        let d = CATALOG.by_id("windows_clipboard_data_files").unwrap();
        assert!(
            d.sources
                .iter()
                .any(|s| s.contains("thinkdfir.com") && s.contains("clippy")),
            "must cite ThinkDFIR Clippy History post — original folder layout research"
        );
    }
}
...
mod tests_linux_pam_d_pamdoora {
    ...
    fn linux_pam_d_related_includes_auth_logs() {
        let d = CATALOG.by_id("linux_pam_d").unwrap();
        let has_auth_log = d.related_artifacts.iter().any(|r| {
            r.contains("auth_log")
                || r.contains("wtmp")
                || r.contains("btmp")
                || r.contains("lastlog")
...
// ── Volatility split / reclassification corrections ──────────────────────────

#[cfg(test)]
mod tests_volatility_corrections {
    use super::*;
    use crate::volatility::VolatilityClass;

    // shimcache: registry value is Persistent; in-memory state is a separate Volatile artifact
    #[test]
    ...
    #[test]
    fn shimcache_memory_exists() {
        assert!(CATALOG.by_id("shimcache_memory").is_some());
    }
    ...
    #[test]
    fn shimcache_memory_related_includes_shimcache() {
        let d = CATALOG.by_id("shimcache_memory").unwrap();
        assert!(d.related_artifacts.contains(&"shimcache"));
    }

    #[test]
    fn shimcache_related_includes_shimcache_memory() {
        let d = CATALOG.by_id("shimcache").unwrap();
        assert!(d.related_artifacts.contains(&"shimcache_memory"));
    }
    ...
    #[test]
    fn linux_dmesg_ring_buffer_exists() {
        assert!(CATALOG.by_id("linux_dmesg_ring_buffer").is_some());
    }

    #[test]
    fn linux_dmesg_ring_buffer_is_volatile() {
        let d = CATALOG.by_id("linux_dmesg_ring_buffer").unwrap();
        assert_eq!(d.volatility, Some(VolatilityClass::Volatile));
    }
    ...
    #[test]
    fn browser_chrome_session_is_activity_driven() {
        let d = CATALOG.by_id("browser_chrome_session").unwrap();
        assert_eq!(d.volatility, Some(VolatilityClass::ActivityDriven));
    }

    // ESXi /var/run/log/* is RAM-backed tmpfs — Volatile, not RotatingBuffer
    #[test]
    fn esxi_attestd_log_is_volatile() {
        let d = CATALOG.by_id("esxi_attestd_log").unwrap();
        assert_eq!(d.volatility, Some(VolatilityClass::Volatile));
    }

    #[test]
    fn esxi_esxtokend_log_is_volatile() {
        let d = CATALOG.by_id("esxi_esxtokend_log").unwrap();
        assert_eq!(d.volatility, Some(VolatilityClass::Volatile));
    }

    #[test]
    fn esxi_kmxa_log_is_volatile() {
        let d = CATALOG.by_id("esxi_kmxa_log").unwrap();
        assert_eq!(d.volatility, Some(VolatilityClass::Volatile));
    }
}
