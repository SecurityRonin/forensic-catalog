...
pub static PRISM: MalwareProfile = MalwareProfile {
    ...
    description: "Minimalist LD_PRELOAD backdoor focused on network hiding. Hooks recvmsg/recvfrom \
    ...
    signals: &[
        ...
        ProfileSignal { id: ARTIFACT_LD_PRELOAD_FOREIGN,     weight: 15, required: true  },
        ProfileSignal { id: ELF_GLOBALLY_LOADED,             weight: 20, required: false },
        ProfileSignal { id: ELF_NOT_IN_PKG_DB,               weight: 15, required: false },
        ProfileSignal { id: ELF_HOOKS_PROCESS_HIDING,        weight: 15, required: false },
        ProfileSignal { id: NETWORK_MAGIC_PACKET_KNOCK,      weight: 25, required: false },
        ProfileSignal { id: TEMPORAL_LDPRELOAD_SSHD_RESTART, weight: 20, required: false },
        ProfileSignal { id: ELF_LIBC_SHADOW_EXPORTS,         weight: 10, required: false },
    ],
    exclusions: &[
        WeightedExclusion { id: ELF_HOOKS_PAM_CREDENTIAL,        penalty: 30 },
...
mod tests {
    ...
    fn prism_required_signals_reach_class_threshold() {
        ...
        assert!(m.score >= PRISM.class_threshold,
            "score {} < class_threshold {}", m.score, PRISM.class_threshold);
    ...
    fn prism_with_globally_loaded_reaches_probable() {
        let s = sigs(&[ELF_HOOKS_NETWORK_HIDING, ARTIFACT_LD_PRELOAD_FOREIGN,
                       ELF_GLOBALLY_LOADED, NETWORK_MAGIC_PACKET_KNOCK]);
        let m = score_against_profile(&s, &PRISM);
        assert!(m.score >= PRISM.probable_threshold,
            "score {} < probable_threshold {}", m.score, PRISM.probable_threshold);
    ...
    fn prism_full_signals_reach_confirmed() {
        let s = sigs(&[ELF_HOOKS_NETWORK_HIDING, ARTIFACT_LD_PRELOAD_FOREIGN,
                       ELF_GLOBALLY_LOADED, ELF_NOT_IN_PKG_DB, ELF_HOOKS_PROCESS_HIDING,
                       NETWORK_MAGIC_PACKET_KNOCK, TEMPORAL_LDPRELOAD_SSHD_RESTART,
    ...
    fn prism_pam_credential_reduces_score() {
        let base = sigs(&[ELF_HOOKS_NETWORK_HIDING, ARTIFACT_LD_PRELOAD_FOREIGN, ELF_GLOBALLY_LOADED]);
        let with_pam = sigs(&[ELF_HOOKS_NETWORK_HIDING, ARTIFACT_LD_PRELOAD_FOREIGN,
                               ELF_GLOBALLY_LOADED, ELF_HOOKS_PAM_CREDENTIAL]);
        let m_base = score_against_profile(&base, &PRISM);
        let m_pam  = score_against_profile(&with_pam, &PRISM);
        assert!(m_pam.score < m_base.score, "PAM credential hook should penalise Prism (no PAM)");
    }

    #[test]
    fn prism_kernel_taint_reduces_score() {
        let base = sigs(&[ELF_HOOKS_NETWORK_HIDING, ARTIFACT_LD_PRELOAD_FOREIGN]);
        let with_lkm = sigs(&[ELF_HOOKS_NETWORK_HIDING, ARTIFACT_LD_PRELOAD_FOREIGN,
                               SYSTEM_KERNEL_TAINT_OOT]);
        let m_base = score_against_profile(&base, &PRISM);
        let m_lkm  = score_against_profile(&with_lkm, &PRISM);
        assert!(m_lkm.score < m_base.score, "kernel taint should penalise Prism (pure LD_PRELOAD)");
    }
}
